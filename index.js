// code segment for changing the values inside the variant and adding my own text or URLs
class ElementHandler {
    ...
    element(element) {
        if (element.tagName == 'title') {
            if (self.flag) {
                element.setInnerContent("Animesh's Website")
            } else {
                element.setInnerContent("Animesh's LinkedIn'")
            }
...
async function ABTesting(request, urls) {
    ...
    if (cookie && cookie.includes(`${NAME}=control`)) {
        ...
        let group = Math.random() < 0.5 ? 'test' : 'control' //return each variant around 50% of the time
